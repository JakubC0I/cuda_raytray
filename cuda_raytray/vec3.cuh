...
public:
    ...
    __device__ static vec3 cuda_random() {
        return vec3(cuda_random_double(), cuda_random_double(), cuda_random_double());
    }
    ...
    __device__ static vec3 cuda_random(double min, double max) {
        return vec3(cuda_random_double(min, max), cuda_random_double(min, max), cuda_random_double(min, max));
    }
};
...
inline vec3 randomo_in_unit_disk() {
    while (true) {
...
__device__ inline vec3 cuda_random_in_unit_disk() {
    while (true) {
        auto p = vec3(cuda_random_double(-1, 1), cuda_random_double(-1, 1), 0);
        if (p.length_squared() < 1) {
            return p;
        }
    }
